...
setwd("G:/My Drive/PhD/project/structural_variant_genotyping_with_pangenome_graph/data_analysis/data/sandbox/")

#args = commandArgs(trailingOnly=TRUE)

five <- read_tsv("ids_and_mapqs.mean.tsv", col_names=c("strain", "vcf_id", "linear_id", "vcf_mapq", "linear_mapq")) %>% 
    mutate(clade=ifelse(strain %in% c("SRR4074413","SRR4074412","SRR4074411","SRR4074394","SRR4074385"), 'paradoxus', 'cerevisiae')) %>%
    mutate(ingraph=ifelse(strain %in% c("SRR4074412", "SRR4074383", "SRR4074385", "SRR4074258", "SRR4074255"), 'included', 'excluded')) %>%
    mutate(graph = "Five strains")

total <- rbind(five, five) %>%
  mutate(diff_id = vcf_id - linear_id) %>%
  mutate(diff_mapq = vcf_mapq - linear_mapq)
